...
try {
  ...
  if ($null -ne $DBParmLine) {
    $KMSKeyID = $DBParmLine.Split('=')[1]
  } else {
    WriteLog -LogFile $LogFile -LogLevel "ERROR" -Log "Failed to retrieve 'CloudKeyUUID' from dbparm.ini, object not found"
    exit 1
  }
  WriteLog -LogFile $LogFile -LogLevel "INFO" -Log "Retrievied KMS Key ID successfully"
  
  # Get Instance ID
  [string]$token = Invoke-RestMethod -Headers @{"X-aws-ec2-metadata-token-ttl-seconds" = "1800"} -Method PUT -Uri http://169.254.169.254/latest/api/token
  ...
  # Import AWS PowerShell module - try v4+ first, then fallback to v3
  $awsModuleLoaded = $false
  try {
    Import-Module -Name AWS.Tools.KeyManagementService -ErrorAction Stop
    Import-Module -Name AWS.Tools.Common -ErrorAction Stop
    $awsModuleLoaded = $true
    WriteLog -LogFile $LogFile -LogLevel "INFO" -Log "AWS Tools for PowerShell (v4+) loaded successfully"
  } catch {
    try {
      Import-Module -Name AWSPowerShell -ErrorAction Stop
      $awsModuleLoaded = $true
      WriteLog -LogFile $LogFile -LogLevel "INFO" -Log "AWS PowerShell (v3) loaded successfully"
    } catch {
      WriteLog -LogFile $LogFile -LogLevel "ERROR" -Log "Failed to load AWS PowerShell module: $_"
      throw "AWS PowerShell module not found. Please install AWS Tools for PowerShell."
    }
  }
  
  # Verify AWS cmdlets are available
  if (-not (Get-Command -Name Add-KMSResourceTag -ErrorAction SilentlyContinue)) {
    throw "Add-KMSResourceTag cmdlet not found. AWS PowerShell module may not be properly installed."
  }
  ...
  # Force load assemblies if needed - sometimes types aren't immediately available after Import-Module
  $kmsAssembly = [System.AppDomain]::CurrentDomain.GetAssemblies() | 
    Where-Object { $_.FullName -like "*Amazon.KeyManagementService*" -or $_.GetName().Name -like "*AWSSDK.KeyManagementService*" } | 
    Select-Object -First 1
  
  if ($null -ne $kmsAssembly) {
    WriteLog -LogFile $LogFile -LogLevel "INFO" -Log "Found KMS assembly: $($kmsAssembly.FullName)"
  }
  ...
  # Create tag object - find the type using reflection if direct creation fails
  $InstanceTag = $null
  $tagTypeName = "Amazon.KeyManagementService.Model.Tag"
  
  try {
    # First, try direct type creation
    $InstanceTag = New-Object -TypeName $tagTypeName -ErrorAction Stop
    $InstanceTag.TagKey = "VaultInstance"
    $InstanceTag.TagValue = $InstanceId
    WriteLog -LogFile $LogFile -LogLevel "INFO" -Log "Created Tag object using direct type name"
  } catch {
    # If that fails, try to find the type in loaded assemblies using reflection
    WriteLog -LogFile $LogFile -LogLevel "INFO" -Log "Direct type creation failed, searching loaded assemblies: $_"
    try {
      # Search all loaded assemblies for the type
      $tagType = $null
      foreach ($assembly in [System.AppDomain]::CurrentDomain.GetAssemblies()) {
        try {
          $tagType = $assembly.GetType($tagTypeName, $false, $true)
          if ($null -ne $tagType) {
            WriteLog -LogFile $LogFile -LogLevel "INFO" -Log "Found Tag type in assembly: $($assembly.FullName)"
            break
          }
        } catch {
          # Continue searching
      ...
      if ($null -ne $tagType) {
        $InstanceTag = [Activator]::CreateInstance($tagType)
        $InstanceTag.TagKey = "VaultInstance"
        $InstanceTag.TagValue = $InstanceId
        WriteLog -LogFile $LogFile -LogLevel "INFO" -Log "Created Tag object using reflection"
      } else {
        throw "Tag type '$tagTypeName' not found in any loaded assemblies"
      }
    } catch {
      WriteLog -LogFile $LogFile -LogLevel "ERROR" -Log "Failed to create Tag object: $_"
      WriteLog -LogFile $LogFile -LogLevel "ERROR" -Log "Please ensure AWS Tools for PowerShell is properly installed."
      WriteLog -LogFile $LogFile -LogLevel "ERROR" -Log "Install command: Install-Module -Name AWS.Tools.KeyManagementService -Force"
      throw "Cannot create KMS Tag object. AWS PowerShell module may not be properly installed or loaded."
    }
  }
  
  # Add the tag - pass as array (required by Add-KMSResourceTag)
  Add-KMSResourceTag -KeyId $KMSKeyID -Tag @($InstanceTag) -Force -Region $Region
  Update-KMSKeyDescription -KeyId $KMSKeyID -Description "CyberArk Digital Vault KMS Key for Vault Instance $InstanceId" -Force -Region $Region
  New-KMSAlias -TargetKeyId $KMSKeyID -AliasName "alias/vault/$InstanceId" -Force -Region $Region
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBCXTCdek38Ocpf
# FEHIFF6WHW+hX3fZal2k3WNtMXUqkKCCGJkwggROMIIDNqADAgECAg0B7l8Wnf+X
...
# AQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIIJUADiyz+fDtF/GPKC/kfb9rqnmE5DM
# dQL/R4YnowTrMA0GCSqGSIb3DQEBAQUABIICAHq81hpL276KVEt6pRGgeNOhRNQn
# 7V7DT41kWb7d7YhQO+rYCVPQGadMjQStZFQzCdH13NUmBNBMBBAAQdc+gDgMfeob
# KlG3n1uWiPZVsJaCUNRcgadu9YjumULEAj9als5K1b8g9BA6n5HzTzMNtkbBp+Zq
# TJPiC9s/qw7/GnASbnipu15ZMKgFWXs5fh141z3M7Enk6c+USNXEFIJwH51fXRUh
# GYZFsv0pj297+C52nJJ+TxsF7PYJnsL8j6fhfUrQr4SNSxlwDNTVbzRm9JMeUxVD
# HWw+B5sM63oBjVBYPCaJ8dm4NVv7VwvjqDUEG7bXlaK2W0hdwhoFdPFgvQyZoF7k
# 0C7Of5gqA2xIF51E0YMToIF4eKqzIn0pzry36wdN2cUSJv78eeODTj0oRs0RuApg
# V1b+npVSrqiawtL7I+bIVyOK/AlPwxVUDFKua1jP7MTytOxFJaD0oRiPK3r5H3X0
# WyHCaFByH3DAMz55niZMdWOqAO5IkmS0aJ2vw8GfKzd/NOlZk58ZkXTsiqc1KpWG
# mpRR7ha8z3R58cWSJ7KLKId42mViQMxIhC4tIplcPSfghgR37A+9BCoNo6jqABPe
# F3DFMtpLnj8KKDu3tkf9bBFfdkjzAOPNVvwCCT5bZ5en97nHOshGZg9Wvdp1KjwQ
# TahpBReurdT28fD0oYId7TCCHekGCisGAQQBgjcDAwExgh3ZMIId1QYJKoZIhvcN
# AQcCoIIdxjCCHcICAQMxDTALBglghkgBZQMEAgIwgeQGCyqGSIb3DQEJEAEEoIHU
# BIHRMIHOAgEBBgsrBgEEAaAyAgMCAjAxMA0GCWCGSAFlAwQCAQUABCCxD/yeX0Nt
# 8SPp4jXpmwEG3uTNen/0dNVxAn4q08RA2gIUcGzqYkjCqvGS+4PzGrQms6PHLvsY
# DzIwMjYwNjE3MDg0NDMyWjADAgEBoF2kWzBZMQswCQYDVQQGEwJCRTEZMBcGA1UE
...
# BAICoQ0GCSqGSIb3DQEBDAUAMD8GCSqGSIb3DQEJBDEyBDBNtUTb0Lum8Ssb4Fdi
# f2x254ZhKKzt/yEw7+oFXyfp/aROVyKvNeTj4CK2ImHEXrUwgbQGCyqGSIb3DQEJ
...
# Q0EgMjAyNQIRAIRyP8GVzBbx2yui9mDfK+QwDQYJKoZIhvcNAQEMBQAEggGAdEzW
# R8oMA8iVfRBChklcPIj5noV4x/+C8Sg+uQW0j8mB4RNnixVwHBXU8LZZ2jgtVxYm
# PtMRiPCkLjC+Z+SK4G38kfXlibQz9QiN8TqVEi3yr2UB6weZORx9EmxIk8Ulvr8p
# bP04qrShh92HWLGZVtqIput+X32Dl3rzaRrClEX/FgzLjVrbJuFE/xbcXLPVz2KW
# 9iKiyk19MC3XW4ANwoqs4VPVAqO+LjHIIWWm3qzGq+xf/uHEJCMsl2QOactibFEv
# 9IScy5xP0M/hWkWJ6HvMV3m5wX18pH30F2nTNtHv5jxabGBwldJDZqbb9vVkNw4g
# NB+XZwDBFx+dau16XVv+E2ue3kIPIB+04szq38pekl+jb39hY0JA2RCUj7wev0IG
# 2cpM/0UTtZxP4l0of/qz+KG/BgQkKDR+5fZy+cvV4Ee6dBHycR4qno/VksgwZDcM
# /g2O5pZ9KQJb8HN+2tLo0tNkxQJhgz85O6cwExCGoh2r+PserXLFiX/6MUA1

...
$DBParmFile='C:\Program Files (x86)\PrivateArk\Server\Conf\dbparm.ini'
$DBParmContent = Get-Content -Path $DBParmFile

try {
    WriteLog -LogFile $LogFile -LogLevel "INFO" -Log "Allowing dbparm.ini rule to Primary Vault"
    $DBParmNewContent = ForEach ($Line in $DBParmContent) {
        if ($Line -like '*169.254.169.123*')
        {
            Write-Output $Line
            Write-Output "AllowNonStandardFWAddresses=[$PrimaryVaultIP],Yes,1858:outbound/tcp"
        }
        else
        {
            Write-Output $Line
    ...
    Set-Content -Path $DBParmFile -Value $DBParmNewContent
    WriteLog -LogFile $LogFile -LogLevel "INFO" -Log "Vault INI Configured Successfully"
} catch {
...
try {
    # Copy License to Conf Folder
    WriteLog -LogFile $LogFile -LogLevel "INFO" -Log "Copying license file to conf folder"
    Copy-Item -Path "C:\CyberArk\Deployment\vaultLicense.xml" -Destination "C:\Program Files (x86)\PrivateArk\Server\Conf\License.xml" -Force
    WriteLog -LogFile $LogFile -LogLevel "INFO" -Log "License copied successfully"
} catch {
    WriteLog -LogFile $LogFile -LogLevel "ERROR" -Log $_.Exception.Message
    exit 1
}
...
try {
    # Start and stop the PrivateArk Server Service to activate new firewall rules
    WriteLog -LogFile $LogFile -LogLevel "INFO" -Log "Activating new firewall rules"
    Start-Service -Name "PrivateArk Server"
    Start-Sleep -Seconds 20
    Stop-Service -Name "PrivateArk Server"
    WriteLog -LogFile $LogFile -LogLevel "INFO" -Log "Firewall rules activated successfully"
} catch {
    WriteLog -LogFile $LogFile -LogLevel "ERROR" -Log $_.Exception.Message
    exit 1
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDFtMj8T65Pf8Fp
# vXrDLkgDoK6gNQQfUEIhjAnnTDJ6xKCCGJkwggROMIIDNqADAgECAg0B7l8Wnf+X
...
# AQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIDL65U9kbZJzPl69FpfCWFhW1ntu6s5n
# SSVOMGVa/UJpMA0GCSqGSIb3DQEBAQUABIICAJFgbTz27M4m4dQQ+88HWJ/2aadw
# S093Cig5wHR0GkZOoUvgllUDNUz2ifeMqIP81OrLoYf8FQOcM9X/4AhpnduDGHQn
# CqingWe0jotCRjS2KZgGVb0G+Fgyav0Nihls2t6qCk2phvm87l2V54EhNGLMkR8J
# dB8yqbW35fOsdbyKT/uzV2O0suUBzq+gIvRQdvgCj63EJxoSELd/zjHXqlzL62GK
# FBJr23oQddwWastlU/bKmd95Dy3OTKNjD3tgtJ8amYW13JW2lJDHwPR4ft4fcWMr
# fy5BsHgT6BD8a36qYKwBFNEk1Nhtjhv6yvqWlK+4Qa0KCHO2HlvsvTIZfmGDe+Cj
# 8wwJzvqNkjAlG04yRI0WkLIYRjOOGif3oqOEvZzjEXZBQI3N+s5xsDk+0AKZ4Um/
# cAGiV0NvqPmW1wwCZo/37lDs3J44LCC9gKhQ9B2JqWv8c+tue+MNGuFF4fCV6q36
# kxPJs3AtNVNaaLvDZkEireftgnSstMkSHsmFUJxnyvEkaRakaTeOkS2v5nOvoocr
# CWgEzf5VOr3MV3VuNDQ/Rq1x9uked3lRSMQeYDHVStwwCuxaNwSUXtvgzyzvtOhh
# Fo8ODIM+xP4yJUWom5A2oYQkDzbeDT/o0u5LiBhBPIG7+UuUf8ajLSIbLYCerqe9
# 51nUmkj9U+MBMvyWoYId7TCCHekGCisGAQQBgjcDAwExgh3ZMIId1QYJKoZIhvcN
# AQcCoIIdxjCCHcICAQMxDTALBglghkgBZQMEAgIwgeQGCyqGSIb3DQEJEAEEoIHU
# BIHRMIHOAgEBBgsrBgEEAaAyAgMCAjAxMA0GCWCGSAFlAwQCAQUABCDoos9JiTGe
# +Lu9VDxKE5ijPeMlw1x1K31dzQjm/SU+OwIUdG+DsSI2U078of/yJpM04L9G+DIY
# DzIwMjYwNjE3MDg0NDE4WjADAgEBoF2kWzBZMQswCQYDVQQGEwJCRTEZMBcGA1UE
...
# BAICoQ0GCSqGSIb3DQEBDAUAMD8GCSqGSIb3DQEJBDEyBDD36+OFWLwhmHovHnBv
# mw3BYBHELUXbRCYzCzr+bUH28/8f+F7rhT8V9IkvOFZ/V4swgbQGCyqGSIb3DQEJ
...
# Q0EgMjAyNQIRAIRyP8GVzBbx2yui9mDfK+QwDQYJKoZIhvcNAQEMBQAEggGAaMLC
# KaZ91W7AjmIHv1dXAtZudrbbHKE0zOBtvQuGc+q2ywQs09YT8pnpGUFc86JrKySx
# 2fteIYPaNuKoiVH4zxkqurMho03aX52DV9mHR4i96/bN+NbRxr2RXlm5ELqtfl/n
# 7bhNg6J/kxUN6E5wtjqVUNz7Nx9eKr3SCFBlOaikY3uCb2DmARJZV5tCKAznEXm4
# w5NqHL7UQc8DtNbr8d6g+wemM8Fa8S67lHdBjw78Z9hPuOxOoCDt1BVXNmTeFVKl
# 248c4p/Ku7sEZA8wejEfeZh1QzgEZq8aCJs9lHT0JFlM4QLkaY7VQEfAkU6T5SpW
# X8iC6bUHogWbhawguKBc2cGhun5IK6RGDuFhMMnp1k1z0Wdw47N5KkAbkJ8A0yNo
# nXUfhazSQrbn+GYyQlnD2Sjlm3fUQiMovUCvf5BDCdLeGgJACITRBcJQ+1gyEd32
# Vkkt7n39sUXmaWWOpbW5wVvxBZIMQDjm5xiT3+LCmlnghE087ZOuUO0w3Qqp

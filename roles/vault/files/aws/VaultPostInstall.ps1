...
param(
  ...
  [Parameter(Mandatory = $false)] [string]$SSMDRPassParameterID,
  [Parameter(Mandatory = $false)] [string]$LicensePath,
  [Parameter(Mandatory = $false)] [string]$RecoveryPublicKeyPath,
  [Parameter(Mandatory = $true)] [string]$IsPrimaryOrDR,
  [Parameter(Mandatory = $false)] [string]$SSMSecretParameterID,
  [Parameter(Mandatory = $false)] [string]$PrimaryVaultIP,
...
$LogFile = "C:\CyberArk\Deployment\Logs\VaultPostInstall.log"

try {
  # Get passwords from SSM
  if($SSMMasterPassParameterID) {
    WriteLog -LogFile $LogFile -LogLevel "INFO" -Log "Getting passwords from SSM"
    $MasterPassword = (Get-SSMParameterValue -Name "$SSMMasterPassParameterID" -WithDecryption $true).Parameters.Value
    WriteLog -LogFile $LogFile -LogLevel "INFO" -Log "Master password retrieved from SSM successfully"
  }
  if($SSMAdminPassParameterID) {
    $AdminPassword = (Get-SSMParameterValue -Name "$SSMAdminPassParameterID" -WithDecryption $true).Parameters.Value
    WriteLog -LogFile $LogFile -LogLevel "INFO" -Log "Admin password retrieved from SSM successfully"
  }
  if ($SSMDRPassParameterID) {
    $DRPassword = (Get-SSMParameterValue -Name "$SSMDRPassParameterID" -WithDecryption $true).Parameters.Value
    WriteLog -LogFile $LogFile -LogLevel "INFO" -Log "DR password retrieved from SSM successfully"
  } else {
    $DRPassword = $AdminPassword
  }
  if($SSMSecretParameterID) {
    $Secret = (Get-SSMParameterValue -Name "$SSMSecretParameterID" -WithDecryption $true).Parameters.Value
    WriteLog -LogFile $LogFile -LogLevel "INFO" -Log "Secret retrieved from SSM successfully"
  }

  Set-Location "C:\Program files (x86)\PrivateArk\Server"

  # Run PostInstall process
  if ($IsPrimaryOrDR -eq 'Primary'){
    WriteLog -LogFile $LogFile -LogLevel "INFO" -Log "Running PostInstall process"
    & ".\CAVaultManager.exe" PostInstall /AdminPass $AdminPassword /MasterPass $MasterPassword /RecPub $RecoveryPublicKeyPath `
    /DRPassword $DRPassword /LicensePath $LicensePath /AcceptEULA /CloudRegion $Region /CloudVendor AWS
    WriteLog -LogFile $LogFile -LogLevel "INFO" -Log "PostInstall process Completed successfully"
    if($Secret) {
      WriteLog -LogFile $LogFile -LogLevel "INFO" -Log "Enabling DR Vault Registration"
      & ".\CAVaultManager.exe" EnableVaultRegistration /AdminUserName  Administrator /AdminPass $AdminPassword /DRUserName DR `
      /Secret $Secret /Expiration 60
      WriteLog -LogFile $LogFile -LogLevel "INFO" -Log "DR Vault Registration for DR is enabled"
    } else {
      WriteLog -LogFile $LogFile -LogLevel "INFO" -Log "DR secret is not set, skipping DR Vault Registration enablement"
    }
    $ServiceName = 'PrivateArk Server'
    WriteLog -LogFile $LogFile -LogLevel "INFO" -Log "Checking if $ServiceName service is running"
    WaitForService -LogFile $LogFile -ServiceName $ServiceName
    # Set ServicesDesiredStartType for primary
    $ServicesDesiredStartType = @{
      "PrivateArk Database" = "Auto"
      "CyberArk Logic Container" = "Auto"
      "PrivateArk Server" = "Auto"
      "Cyber-Ark Event Notification Engine" = "Auto"
    }
  } else {
    Set-Location "C:\Program files (x86)\PrivateArk\PADR"
    WriteLog -LogFile $LogFile -LogLevel "INFO" -Log "Registering DR to vault using PADR"
    & ".\PADR.exe" RegisterVault /PRIMARYVAULTIP $PrimaryVaultIP /DRUSERNAME $DRUser /DRUSERPASSWORD $DRPassword /DRUSERSECRET $Secret /CLOUDVENDOR AWS `
    /ENABLEFAILOVER  /ACCEPTEULA /CLOUDREGION $Region
    WriteLog -LogFile $LogFile -LogLevel "INFO" -Log "DR registered successfully"
    $ServiceName = 'CyberArk Vault Disaster Recovery'
    WriteLog -LogFile $LogFile -LogLevel "INFO" -Log "Starting $ServiceName service"
    Start-Service -Name $ServiceName
    WriteLog -LogFile $LogFile -LogLevel "INFO" -Log "Checking if $ServiceName service is running"
    WaitForService -LogFile $LogFile -ServiceName $ServiceName
    # Set ServicesDesiredStartType for dr
    $ServicesDesiredStartType = @{
      "PrivateArk Database" = "Auto"
      "CyberArk Logic Container" = "Manual"
      "PrivateArk Server" = "Manual"
      "Cyber-Ark Event Notification Engine" = "Manual"
      "CyberArk Vault Disaster Recovery" = "Auto"
    }
  }
  # Execute validateServicesStartType commands
  WriteLog -LogFile $LogFile -LogLevel "INFO" -Log "Validating services start type"
  foreach ($ServiceName in $ServicesDesiredStartType.Keys) {
    ...
    if ($null -ne $Service) {
      ...
      # Compare the current start type with the desired start type
      if ($CurrentStartType -eq $ServicesDesiredStartType[$ServiceName]) {
        WriteLog -LogFile $LogFile -LogLevel "INFO" -Log "Service '$ServiceName' is configured correctly: $CurrentStartType"
      } else {
        WriteLog -LogFile $LogFile -LogLevel "ERROR" -Log "Service '$ServiceName' is NOT configured correctly. Current: $CurrentStartType, Expected: $($ServicesDesiredStartType[$ServiceName])"
        exit 1
      }
    } else {
      WriteLog -LogFile $LogFile -LogLevel "ERROR" -Log "Service '$ServiceName' was not found."
      exit 1
    }
  }
  WriteLog -LogFile $LogFile -LogLevel "INFO" -Log "Services start type validation completed successfully"
} catch {
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCD9KWxrgtZhQetD
# DS/qaBHvsrapIJmOIjqOS0ZhjI5iV6CCGJkwggROMIIDNqADAgECAg0B7l8Wnf+X
...
# AQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIJdXdCwgJf1T87b5tFEjRIVt2oA+9w22
# uD9hWmtVh4v0MA0GCSqGSIb3DQEBAQUABIICADbsUatLjaLcSycGdKxhLBHLL7Ax
# oS9vibRnSThh4x0zffh+aCEbP1+a0mqTsQ5i8uLoQznVUeCBkzjZOVBiEPoU1IMB
# DGT5oIFUtsqC7iFR3hVBsChDWtLbL4VuBy7EQ8nmNWPYLIMpkie+Fhv6JQ5ADgH+
# 4msW8TO1RahLhhiWEX3D/Ccpeb9OT9gDU5/sSSF8QXl2qZ91S8zjq6Er92O/6YE8
# KLuHSstalTfDlCC4ZKRaRYSNGO4NjuerlyB1QtgqBt5kFLgkKqOJTofjAH8dFZYX
# PR6ejtj99usMBzMo9mcglZrsI//397gMbdG4CNHQ6Lmr5va2IDouPReKjelqFry+
# a+vQniFTDYhKW2u3RT91a80LSfUkYGSs1f6w/mlFnp7DSGkegVG5jTv/gZULIn0O
# SmMxL5RvplXnSuPliWDITSE/xnqNgQF1FG+8fY24wrXml2ADSVGumW6aGrRa9kMG
# FyhSpxQyM3PXJU6rqWyJuCFMk5bSTFFpj/IF0zgsnexKvJDJ0SxX6cQJKP7fdgg6
# 7CbpE1aQNI4FUhyRWmjlOpNHe0wud4sOucBtkv/FJRfHu7KjTyh1Px1wo6LnXCwH
# DumE/S6+eexdd5MbNAMF8cLB5ZD/4q4mmBZDlC4g8rTes+h0kkgRNQb6q5YWi86H
# n/LoJNt/KKKjBBLToYId7TCCHekGCisGAQQBgjcDAwExgh3ZMIId1QYJKoZIhvcN
# AQcCoIIdxjCCHcICAQMxDTALBglghkgBZQMEAgIwgeQGCyqGSIb3DQEJEAEEoIHU
# BIHRMIHOAgEBBgsrBgEEAaAyAgMCAjAxMA0GCWCGSAFlAwQCAQUABCDu9NC3djPD
# l1lwByEv6qZjBx3lrPhvc85sP7JU0gGgmgIUf1lGN3E/wcNsC+8syZcx5Lrrg0cY
# DzIwMjYwNjE3MDg0NDM0WjADAgEBoF2kWzBZMQswCQYDVQQGEwJCRTEZMBcGA1UE
...
# BAICoQ0GCSqGSIb3DQEBDAUAMD8GCSqGSIb3DQEJBDEyBDAMpObgEyIHzYsRWR49
# /87TBVCBbz1BcLrzfjvLhVjjhSawkEhzD8N8u76PYNNuyYAwgbQGCyqGSIb3DQEJ
...
# Q0EgMjAyNQIRAIRyP8GVzBbx2yui9mDfK+QwDQYJKoZIhvcNAQEMBQAEggGAvXcg
# Xd8jprHPZqISyVs1fbOn2tJqzkzSlEwPMJAaGjil4XAP38DJHX6ZNdcqsew1gWBP
# P47lIsTW25cGerACFWz7YO0b5Uodh/ji8j1YZY11NF3SeyCVrlSqnjgrxLqBPK60
# cd5Y49LJ30CD+zlN5C7R+/8O+dQOG3CCg6Zi5c9m9kHHasZeOo4gfo1sf9aDBC2e
# 9ejIwW498+s8TPW6br8uGVnhrfpLdjYCHgkm9cKtNByXCBG2mp3K9SDlLXX/tUZU
# ynpVnYvMm3nWMMyPVGsjLIyyCMRWiSqrOswF98nORNl5tF27I/oMJB8WwSXlbhZk
# REceZiISCwO2lGUA1+B1LcGbzAlI0YV86r2Pk0cAqDQWpgFSCa89UXlR6EXoBJ9i
# LaTUagU1gqHHU4B3z8tQ1xHnYbkX5cfb0JifNI7nSA0wXLubpwWIyTmAYY64awR1
# oLLvFtqUYBmryms4Q5Qz/y7J0Xy6scKoSsNCXvZ/uuUagY941yxHRBwrsaxS

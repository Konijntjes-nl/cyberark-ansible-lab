[CmdletBinding()]
  Param(
    ...
    [Parameter(Mandatory=$false)][String]$MasterPass,
    [Parameter(Mandatory=$true)][string]$PrimaryOrDR,
    [Parameter(Mandatory=$true)][string]$PrimaryVaultIP,
    [Parameter(Mandatory=$false)][String]$DRPassword,
    [Parameter(Mandatory=$false)][string]$LicenseFileName,
    [Parameter(Mandatory=$false)][string]$RecPubFileName,
    ...
    [Parameter(Mandatory=$true)][string]$VKMName,
    [Parameter(Mandatory=$false)][string]$DRUser
  )
. "$PSScriptRoot\Common.ps1"
$LogFile = "C:\CyberArk\Deployment\Logs\HardeningActivation.log"
$ErrorActionPreference = "SilentlyContinue"
$ServerFolder = "C:\Program Files (x86)\PrivateArk\Server"
$PADRFolder = "C:\Program Files (x86)\PrivateArk\PADR"
$HardeningFolder = "$ServerFolder\Hardening"
$VaultMgrLogFile = "$ServerFolder\Logs\CAvaultManager.log"
$PADRLogFile = "$PADRFolder\Logs\padr.log"
$DRUser = "DR"

### Download license.xml and recpub.key from blob
try {

  # Set the Azure cloud environment (AzureCloud or AzureUSGovernment)
  ...
  if ($metadata.compute.azEnvironment -like '*usgov*') { 
    $azureCloudEnv = 'AzureUSGovernment'
    $keyVaultDnsSuffix = 'vault.usgovcloudapi.net'
  } else { 
    $azureCloudEnv = 'AzureCloud'
    $keyVaultDnsSuffix = 'vault.azure.net'
  }
    
  if ($PrimaryOrDR -eq 'Primary'){
    # Get authentication method to storage account:
    if (($StorageAccountKey -ne $null) -and ($StorageAccountKey -ne "")) {  
      # Use storage account access key to get storage account context:
      $storageContext = New-AzStorageContext $StorageName -StorageAccountKey $StorageAccountKey -Environment $azureCloudEnv
    } else {
      # Use system assigned managed identity to get storage account context:
      Connect-AzAccount -Identity -Environment $azureCloudEnv
      $resourceGroupName = (Get-AzStorageAccount | Where-Object { $_.StorageAccountName -eq $StorageName }).ResourceGroupName
      $storageContext = (Get-AzStorageAccount -ResourceGroupName $resourceGroupName -Name $StorageName).Context
    }
    # Download the files:
    WriteLog -LogFile $LogFile -LogLevel "INFO" -Log "Downloading license.xml and recpub.key from blob"
    Get-AzStorageBlobContent -Container $ContainerName -Blob $LicenseFileName -Destination "C:\vaultLicense.xml" -Context $storageContext
    Get-AzStorageBlobContent -Container $ContainerName -Blob $RecPubFileName -Destination "C:\recoveryPublic.key" -Context $storageContext
  }

  ### Create Log Files for Hardening and PostInstall
  If (!(Test-Path $LogFile)) {
    New-Item -Path $LogFile
  }

  if ($VKMName) {
    # Build DNS String from VKMNAME
    $VKMDNS = "https://$VKMName.$keyVaultDnsSuffix/"
  }
...
try {
  ...
  while ($Retries -lt 3) {
    if ($Success -eq $True) {
      WriteLog -LogFile $LogFile -LogLevel "INFO" -Log "Activation completed after $Retries times"
      break
    } else {
      $Retries = $Retries + 1
      echo "Retry number $Retries"
      if ($PrimaryOrDR -eq 'Primary'){
        WriteLog -LogFile $LogFile -LogLevel "INFO" -Log "Trying to harden and activate vault"
        $output = & ".\CAVaultManager.exe" PostInstall /AdminPass $AdminPass /MasterPass $MasterPass /RecPub "C:\recoveryPublic.key" `
        /DRPassword $DRPassword /LicensePath "C:\vaultLicense.xml" /CloudVendor "AZURE" /VKMName $VKMDNS `
        /ACCEPTEULA /RDPGateway 10.10.10.10 2>&1
        $Success = Select-String -Path $VaultMgrLogFile -Pattern "ITATP146I" -Quiet
         if($LASTEXITCODE -ne 0){
            throw $output
          }
        if ($Secret.Length -ge 10 -and `
            $Secret -match '^([\w+\d+[!@#$%^&*~?\(\)]+[^\\\/\-\;\n]+)$' -and `
            $Secret -match '(.{0,}[!@#$%^&*~?\(\)]+.{0,})' -and `
            $Secret -match '(.{0,}[a-zA-Z]+.{0,})' -and `
            $Secret -match '(.{0,}[\d]+.{0,})'){
          WriteLog -LogFile $LogFile -LogLevel "INFO" -Log "Enabling DR registration"
          $output = & ".\CAVaultManager.exe" EnableVaultRegistration /AdminUserName  Administrator /AdminPass $AdminPass /DRUserName DR `
          /Secret $Secret /Expiration 60 2>&1
          $Success = Select-String -Path $VaultMgrLogFile -Pattern "CAVLT326I" -Quiet
          if($LASTEXITCODE -ne 0){
            throw $output
          }
          WriteLog -LogFile $LogFile -LogLevel "INFO" -Log "DR enabled successfully"
        }elseif($Secret -eq '' -or $Secret -eq $null){
            WriteLog -LogFile $LogFile -LogLevel "INFO" -Log "Skipping DR registration"
        }elseif($Secret -ne ''){
            WriteLog -LogFile $LogFile -LogLevel "Error" -Log "The secret provided has failed validation. Secret must contain 10 characters including:`
            - one or more letters (a-Z)`
            - one or more numbers(0-9)`
            - one or more special characters : !@#$%^&*~?()`
            Secret cannot contain: ^\/-;"
        }else{
            WriteLog -LogFile $LogFile -LogLevel "Error" -Log "failed to enable DR registration "
        }
      }else{
        WriteLog -LogFile $LogFile -LogLevel "INFO" -Log "Registering DR"
        Set-Location "C:\Program files (x86)\PrivateArk\PADR"
        $output = & ".\PADR.exe" RegisterVault /PRIMARYVAULTIP $PrimaryVaultIP /DRUSERNAME $DRUser /DRUSERPASSWORD $DRPassword /DRUSERSECRET $Secret /CLOUDVENDOR Azure `
        /ENABLEFAILOVER  /ACCEPTEULA /VKMName $VKMDNS /RDPGATEWAYIP 10.10.10.10  2>&1
        $Success = Select-String -Path $PADRLogFile -Pattern "ITATP146I" -Quiet
         if($LASTEXITCODE -ne 0){
            throw $output
          }
        WriteLog -LogFile $LogFile -LogLevel "INFO" -Log "DR registered successfully"
      }
    }
  }
   if ($Retries -eq 3) {
     WriteLog -LogFile $LogFile -LogLevel "INFO" -Log "Hardening failed"
     exit 1
   }
} catch {
  WriteLog -LogFile $LogFile -LogLevel "ERROR" -Log $_.Exception.Message
...
# When deploying Vault and VaultDR machines on Azure, after last restart, the main service would not start normally 
# and it would affect other deployed machines, so making sure the service is up after restart is critical.
try {
  # Configure a scheduled task to make sure Vault or VaultDR main services are up (3 retries), according to machine
  if ($PrimaryOrDR -eq 'Primary') {
  ...
  WriteLog -LogFile $LogFile -LogLevel "INFO" -Log "Configuring a scheduled task to verify service $serviceName is up after reboot"
  ...
  $scriptBlock = @"
    ...
    while (`$attempt -le `$maxretries -and -not `$success) {
      WriteLog -LogFile $LogFile -LogLevel "INFO" -Log "Waiting a few seconds before checking status of $serviceName service..."
      ...
      if (`$service) {
        if (`$service.Status -ne 'Running') {
          ...
          WriteLog -LogFile $LogFile -LogLevel "INFO" -Log "$serviceName service is not running, trying to start it now (attempt #`$attempt)"
          Start-Service -Name "$serviceName"
          WriteLog -LogFile $LogFile -LogLevel "INFO" -Log "$serviceName service has been started"
          `$attempt++
        } else {
          WriteLog -LogFile $LogFile -LogLevel "INFO" -Log "$serviceName service is running"
          `$success = `$true
        }
      } else {
        WriteLog -LogFile $LogFile -LogLevel "ERROR" -Log "$serviceName service does not exist"
        break
      }
    }

    # Remove scheduled task
    Unregister-ScheduledTask -TaskName "VerifyServices" -Confirm:`$false

    WriteLog -LogFile $LogFile -LogLevel "INFO" -Log "Scheduled task to verify service $serviceName is up after reboot was removed"
"@
  ...
  Register-ScheduledTask `
    ...
    -RunLevel "Highest" `
    -TaskName "VerifyServices" `
    -Description "Verify $serviceName service is running after reboot"
} catch {
  WriteLog -LogFile $LogFile -LogLevel "WARN" -Log $_.Exception.Message
}

### Restart Computer to ensure services are down
Restart-Computer -Force
# SIG # Begin signature block
# MII6SQYJKoZIhvcNAQcCoII6OjCCOjYCAQExDzANBglghkgBZQMEAgEFADB5Bgor
# BgEEAYI3AgEEoGswaTA0BgorBgEEAYI3AgEeMCYCAwEAAAQQH8w7YFlLCE63JNLG
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCxOlio6eE4Zrqq
# X/EwvHvazUEQScgJvJGmKSRCDMZTDKCCGJkwggROMIIDNqADAgECAg0B7l8Wnf+X
...
# AQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEICiWlMAlYmO0WO81eS8WFUsFIGBpSjCR
# fSofsbUe33FnMA0GCSqGSIb3DQEBAQUABIICAKx0CMEZVSmi7dFcSBrnoGIN79ak
# crtoGJuzGqB9oojyAV734lcMv+4N+6Zbzd2C2s8Pn6nMSJg0W+vfMCD2sqw8qI+P
# J2A/GlxBQ9BIBn9Z+hhyUcZphwcW9lGhRwvk4guZ5ZSOd0wWEZG/pDDFoRKfDh9p
# wRBA4+zxTz//vW6YAvZpQzHChrK5wh0XwsH8T6Bbz1J54QsJs+oZcYBBTkA8FNdm
# JScoTZcOC7gOR4LGNkLPeYZsd+fGL4ZDcTWD8YXH952xHs3trmjEHmOWv7e1lhaX
# SlFQVvtG/bke0np3K+q57Pvqe25xolj+Mapnc+Eji5apkYn+T2Ydos39ZzWTDMcb
# TxVJ80jxiauCP7VOMk9qEpYY3b9Gg/A/Mz2tOY/apNDdBH7KEwMG1vjcEQxk6+rr
# 6o6G3SBecQdrOmdZwGMxyMyXpk9epXNCYGnRvLgtYRz5lUJsj+jmwcwJGR6KLxUl
# 4cMy5WROdLXLINT71lbhZFHvrI0rOqKxEvPqgMYYwd1RweHZqApBwzWyyBhLUEac
# 7EZ2+xhqLd9oWn7Db/UOWf47fs6swv06pllvnhfFXgX8yPLhV6YNlzNk0nOJ3S7K
# iGx7qfVhMqfKddY8VZ6gAY9qHUryvjDnH7Ocwg44krL73ZvKD/mnsqR/z3n8BmUJ
# DZjtdigMFp1/XYUWoYId7TCCHekGCisGAQQBgjcDAwExgh3ZMIId1QYJKoZIhvcN
# AQcCoIIdxjCCHcICAQMxDTALBglghkgBZQMEAgIwgeQGCyqGSIb3DQEJEAEEoIHU
# BIHRMIHOAgEBBgsrBgEEAaAyAgMCAjAxMA0GCWCGSAFlAwQCAQUABCDL2c7R7OYC
# EXffKLagiI8qS1DIpCpeqJz5ipNZtu9dDAIUeEdhUDvVVqswbcEvfUP24M3uax0Y
# DzIwMjYwNjE3MDg0NDQzWjADAgEBoF2kWzBZMQswCQYDVQQGEwJCRTEZMBcGA1UE
...
# BAICoQ0GCSqGSIb3DQEBDAUAMD8GCSqGSIb3DQEJBDEyBDC6qkC2Ld0x/Y6qchjS
# 7t28Qi7hMfxnPZcqdRS7R/VKh2zJeg5HQv5DGm8O1WXES48wgbQGCyqGSIb3DQEJ
...
# Q0EgMjAyNQIRAIRyP8GVzBbx2yui9mDfK+QwDQYJKoZIhvcNAQEMBQAEggGArSlz
# wvnC+3wGYrXlm24uhIveIMeizW7aoTx/dpF71q5uYJZtQazLtv0nNGm6xXIm33Z3
# ZMiPrfKZJ3JqNnsdxZnoVut9h06i1RBKgd7VaOWTQ9V2ZCCR9i/ItBl2kU0ucvxa
# 3vZxwjE2a6xBkQNscofLFqz9Wv922dKhRmsa/YEkWitakA3BYms9EJSiryiA53Zn
# haZ4XQcUhPi5LEcDnW6+k9YGQ0tv+RZ3LuvmsWvgDXzM4vimlzQBw91mO5SuKh1q
# EQLXQOYUFDai+4/6GwL18KUNAMFH3nKB8x5fB3GTP9azndSKKh/jVefpmjveZaEc
# TC5r54EcHNncidMATg+MUAP9Fnq2PWZRZGL/dw6q9Lglv3Oi+wjZN5kB1YWLxaWb
# MLEETCQ1JZRtifboCZarprLBzZ4l28yFwto2xo2BwLmHKKImJL10fKWeFWhb99Bl
# I+BHkisgFVR3m77ue0oQ8aTa1r5kWXntjofayrPQKgO+Gn1XXoiYyvkxrqMB
